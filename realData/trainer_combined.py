...
if is_cuda: device = torch.device("cuda:1")
...
def train(path, is_saved, regularization_param, epochs):
    ...
    if is_saved == 1: model.load_state_dict(torch.load(PATH1))
    ...
    torch.save(model.state_dict(), os.path.join(r'./saved_models/', 'modelLSTM_GCN_MLP'))
